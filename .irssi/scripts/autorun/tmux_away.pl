...
$VERSION = "0.9.7";
%IRSSI = (
    authors     => 'Matt Mullins <mmullins@mmlx.us>, Andreas \'ads\' Scherbaum <ads@wars-nicht.de>',
    name        => 'tmux_away',
...
# ported to tmux by Matt Mullins <mmullins@mmlx.us>
#
# changes:
#  25.07.2011 ported to tmux
...
# try to find out, if we are running in tmux
...
# search for socket
# the TMUX environment variable is socket-path,number,number
my $socket = $ENV{'TMUX'};
...
if ($socket =~ /^(.*),\d+,\d+/s) {
  ...
  Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'tmux_away_crap',
    "error reading tmux socket information from:");
  Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'tmux_away_crap',
    "$socket");
  return;
}
